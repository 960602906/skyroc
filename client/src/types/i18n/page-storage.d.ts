declare namespace App {
  namespace I18n {
    interface PageStorage {
      in: {
        add: string;
        audit: string;
        businessStatus: string;
        details: string;
        edit: string;
        expectedArrivalTime: string;
        form: {
          businessStatus: string;
          detailRemark: string;
          expectedArrivalTime: string;
          expireDate: string;
          goodsId: string;
          goodsUnitId: string;
          inTime: string;
          keyword: string;
          productDate: string;
          purchaserId: string;
          quantity: string;
          remark: string;
          supplierId: string;
          unitPrice: string;
          wareId: string;
        };
        goods: string;
        inNo: string;
        inTime: string;
        other: {
          add: string;
          addDetail: string;
          audit: string;
          auditTime: string;
          auditUserName: string;
          back: string;
          batchNo: string;
          businessStatus: string;
          createTime: string;
          department: string;
          edit: string;
          editDetail: string;
          expireDate: string;
          form: {
            businessStatus: string;
            keyword: string;
            wareId: string;
          };
          goods: string;
          goodsCode: string;
          goodsName: string;
          goodsUnitName: string;
          inNo: string;
          inTime: string;
          keyword: string;
          productDate: string;
          quantity: string;
          reverseAudit: string;
          reverseAuditBtn: string;
          reverseTime: string;
          reverseUserName: string;
          sectionAudit: string;
          sectionBasic: string;
          sectionDetails: string;
          title: string;
          totalAmount: string;
          totalPrice: string;
          unitPrice: string;
          updateTime: string;
          ware: string;
        };
        purchase: {
          add: string;
          addDetail: string;
          audit: string;
          auditTime: string;
          auditUserName: string;
          back: string;
          batchNo: string;
          businessStatus: string;
          createTime: string;
          department: string;
          edit: string;
          editDetail: string;
          expireDate: string;
          form: {
            businessStatus: string;
            keyword: string;
            supplierId: string;
            wareId: string;
          };
          goods: string;
          goodsCode: string;
          goodsName: string;
          goodsUnitName: string;
          inNo: string;
          inTime: string;
          keyword: string;
          productDate: string;
          purchasePattern: string;
          purchaser: string;
          quantity: string;
          reverseAudit: string;
          reverseAuditBtn: string;
          reverseTime: string;
          reverseUserName: string;
          sectionAudit: string;
          sectionBasic: string;
          sectionBusiness: string;
          sectionDetails: string;
          supplier: string;
          title: string;
          totalAmount: string;
          totalPrice: string;
          unitPrice: string;
          updateTime: string;
          ware: string;
        };
        purchasePattern: string;
        purchaser: string;
        remark: string;
        reverseAudit: string;
        salesReturn: {
          add: string;
          addDetail: string;
          afterSale: string;
          afterSaleHint: string;
          afterSaleId: string;
          audit: string;
          auditTime: string;
          auditUserName: string;
          back: string;
          batchNo: string;
          businessStatus: string;
          createTime: string;
          customer: string;
          department: string;
          edit: string;
          editDetail: string;
          expireDate: string;
          form: {
            afterSaleId: string;
            businessStatus: string;
            customerId: string;
            keyword: string;
            wareId: string;
          };
          goods: string;
          goodsCode: string;
          goodsName: string;
          goodsUnitName: string;
          importPickupTasks: string;
          importSuccess: string;
          inNo: string;
          inTime: string;
          keyword: string;
          pickupTaskId: string;
          productDate: string;
          quantity: string;
          reverseAudit: string;
          reverseAuditBtn: string;
          reverseTime: string;
          reverseUserName: string;
          sectionAudit: string;
          sectionBasic: string;
          sectionDetails: string;
          source: string;
          sourceAfterSale: string;
          sourceManual: string;
          taskNo: string;
          title: string;
          totalAmount: string;
          totalPrice: string;
          unitPrice: string;
          updateTime: string;
          validation: {
            noEligiblePickupTasks: string;
            pickupTaskLocked: string;
            selectAfterSaleFirst: string;
            useImportForAfterSale: string;
          };
          ware: string;
        };
        statusAudited: string;
        statusDeleted: string;
        statusDraft: string;
        statusPendingAudit: string;
        statusReversed: string;
        supplier: string;
        totalAmount: string;
        validation: {
          atLeastOneDetail: string;
        };
        ware: string;
        wareId: string;
      };
      ware: {
        address: string;
        addWare: string;
        code: string;
        contactName: string;
        contactPhone: string;
        createTime: string;
        detail: {
          back: string;
          createTime: string;
          title: string;
          updateTime: string;
        };
        editWare: string;
        form: {
          address: string;
          code: string;
          contactName: string;
          contactPhone: string;
          name: string;
          remark: string;
          sort: string;
          status: string;
        };
        name: string;
        remark: string;
        sectionBasic: string;
        sectionStatus: string;
        sort: string;
        status: string;
        title: string;
      };
    }
  }
}
