...
const MenuOperateModal = ({ allPages, form, handleSubmit, menuList, onClose, open, operateType }: Props) => {
  ...
  const parentId = useMemo(() => form.getFieldValue('parentId'), []);
  const { hideInMenu, icon, iconType, menuType, name } =
  ...
  const showLayout = menuType === MenuType.DIRECTORY || !parentId;

  const pageOptions = getPageOptions(name, allPages);
        ...
        <SimpleScrollbar>
          ...
          >
            ...
            <ARow>
              ...
              <ACol span={24}>
                <AForm.Item
                  label={t('page.manage.menu.buttons')}
                  labelCol={{ span: 4 }}
                >
                  <AForm.List name="buttons">
                    {(subFields, { add, remove }) => {
                      return (
                        <>
                          {subFields.map(item => (
