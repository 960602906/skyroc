...
import {
  ...
  fetchDeleteCompany,
  fetchGetCompanyDetail,
  fetchGetCompanyList,
  fetchToggleCompanyStatus,
  fetchUpdateCompany
} from '@/service/api';

import CompanySearch from './modules/CompanySearch';
...
const CompanyManage = () => {
  ...
  const { columnChecks, data, run, searchProps, setColumnChecks, tableProps } = useTable({
    ...
    columns: () => [
      ...
      },
      {
        ...
        key: 'contactName',
        minWidth: 100,
        title: t('page.customer.company.contactName')
      },
      {
        align: 'center',
        dataIndex: 'contactPhone',
        key: 'contactPhone',
        title: t('page.customer.company.contactPhone'),
        width: 130
      },
      {
        dataIndex: 'address',
        ...
        key: 'address',
        minWidth: 160,
        title: t('page.customer.company.address')
      },
      {
        align: 'center',
        dataIndex: 'status',
      ...
      },
      {
        ...
        key: 'createTime',
        title: t('page.customer.company.createTime'),
        width: 170
      },
      {
        align: 'center',
  ...
  const { checkedRowKeys, generalPopupOperation, handleAdd, handleEdit, onBatchDeleted, onDeleted, rowSelection } =
    useTableOperate(data, run, async (res, type) => {
      if (type === 'add') {
        await fetchAddCompany(res);
      } else {
        await fetchUpdateCompany(res);
      }
    });

  async function handleBatchDelete() {
    await fetchBatchDeleteCompany(checkedRowKeys.map(key => key as string));
    onBatchDeleted();
  }
  ...
  async function handleToggleStatus(record: Api.Company.Entity) {
    ...
    await toggleEntityStatus({
      ...
      refresh: run,
      t,
      toggleFn: fetchToggleCompanyStatus
    });
  }

  return (
    <CrudPageLayout
      search={<CompanySearch {...searchProps} />}
      tableWrapperRef={tableWrapperRef}
      title={t('page.customer.company.title')}
      extra={
      ...
      table={
        <>
          ...
          <Suspense>
            <CompanyOperateDrawer {...generalPopupOperation} />
          </Suspense>
        </>
      }
    />
