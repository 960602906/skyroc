...
public static class DatabaseCommentExtensions
{
    ...
    {
        [typeof(AiConversation)] = "AI 助手会话，按系统用户隔离并控制消息保留期限",
        [typeof(AiMessage)] = "AI 会话消息，只保存最终文字及脱敏后的工具和来源摘要",
        [typeof(AiActionDraft)] = "AI 通用写操作草稿，绑定用户、operationId 和规范化参数并等待人工确认",
        [typeof(AiOrderDraft)] = "AI 生成的销售订单草稿，必须由所属用户在有效期内人工确认",
        [typeof(AiOrderDraftDetail)] = "AI 订单草稿商品明细，保存确认前重新校验所需的价格与单位快照",
        [typeof(McpAccessToken)] = "个人 MCP 访问令牌元数据，只保存非敏感前缀和 HMAC-SHA256 哈希",
    ...
    private static readonly IReadOnlyDictionary<(Type EntityType, string PropertyName), string> EntityPropertyComments =
        ...
        {
            [(typeof(AiConversation), nameof(AiConversation.UserId))] = "会话所属系统用户主键，是所有会话查询的隔离边界",
            [(typeof(AiConversation), nameof(AiConversation.Title))] = "会话列表展示标题，不包含模型推理内容",
            [(typeof(AiConversation), nameof(AiConversation.ConversationStatus))] = "会话状态：1 活动，2 用户已删除并等待清理",
            [(typeof(AiConversation), nameof(AiConversation.LastMessageTime))] = "最近一条会话消息写入时间（UTC）",
            [(typeof(AiConversation), nameof(AiConversation.RetainUntil))] = "会话及其消息最早可清理时间（UTC），默认保留 30 天",
            [(typeof(AiConversation), nameof(AiConversation.DeletedTime))] = "用户删除会话的时间（UTC），活动会话为空",
            [(typeof(AiMessage), nameof(AiMessage.ConversationId))] = "所属 AI 会话主键",
            [(typeof(AiMessage), nameof(AiMessage.Role))] = "消息角色：1 系统，2 用户，3 助手，4 工具摘要",
            [(typeof(AiMessage), nameof(AiMessage.Content))] = "最终可展示文字，不得包含模型推理或完整敏感工具结果",
            [(typeof(AiMessage), nameof(AiMessage.Sequence))] = "消息在会话内单调递增的游标序号",
            [(typeof(AiMessage), nameof(AiMessage.MessageStatus))] = "消息状态：1 生成中，2 已完成，3 失败，4 已取消",
            [(typeof(AiMessage), nameof(AiMessage.ProviderName))] = "生成助手消息所用的统一 Provider 名称",
            [(typeof(AiMessage), nameof(AiMessage.ModelName))] = "生成助手消息所用的模型名称",
            [(typeof(AiMessage), nameof(AiMessage.ToolCallId))] = "工具调用在当前模型回合中的标识",
            [(typeof(AiMessage), nameof(AiMessage.ToolName))] = "被调用的白名单工具名称",
            [(typeof(AiMessage), nameof(AiMessage.ToolStatus))] = "工具执行状态的稳定文本值",
            [(typeof(AiMessage), nameof(AiMessage.ToolSummary))] = "工具结果的必要脱敏摘要，不保存完整业务响应",
            [(typeof(AiMessage), nameof(AiMessage.SourceReferences))] = "知识来源 JSON 数组，只允许来源 slug 和标题",
            [(typeof(AiMessage), nameof(AiMessage.CompletedTime))] = "消息进入完成、失败或取消终态的时间（UTC）",
            [(typeof(AiActionDraft), nameof(AiActionDraft.ConversationId))] = "来源 AI 会话主键，外部 MCP 独立生成时为空",
            [(typeof(AiActionDraft), nameof(AiActionDraft.UserId))] = "草稿所属用户主键，也是唯一允许确认和执行的用户",
            [(typeof(AiActionDraft), nameof(AiActionDraft.OperationId))] = "能力目录中的稳定 operationId，确认和执行时不得改变",
            [(typeof(AiActionDraft), nameof(AiActionDraft.CanonicalArgumentsJson))] = "属性按序且无额外空白的规范化业务参数 JSON",
            [(typeof(AiActionDraft), nameof(AiActionDraft.ArgumentsHash))] = "绑定所属用户、operationId 和规范化参数的 SHA-256 哈希",
            [(typeof(AiActionDraft), nameof(AiActionDraft.RiskLevel))] = "草稿风险等级：1 普通写操作，2 高风险操作",
            [(typeof(AiActionDraft), nameof(AiActionDraft.ConfirmationSummary))] = "展示给用户的最小业务变更摘要，不含密钥或完整敏感响应",
            [(typeof(AiActionDraft), nameof(AiActionDraft.DraftStatus))] = "草稿状态：1 待确认，2 已确认，3 执行中，4 已执行，5 失败，6 过期，7 取消",
            [(typeof(AiActionDraft), nameof(AiActionDraft.ExpiresAt))] = "草稿失效时间（UTC），默认自生成起 30 分钟",
            [(typeof(AiActionDraft), nameof(AiActionDraft.ConfirmedByUserId))] = "实际确认草稿的系统用户主键，必须与草稿所属用户一致",
            [(typeof(AiActionDraft), nameof(AiActionDraft.ConfirmedTime))] = "草稿通过人工确认的时间（UTC）",
            [(typeof(AiActionDraft), nameof(AiActionDraft.ExecutedTime))] = "原业务接口执行结束的时间（UTC）",
            [(typeof(AiActionDraft), nameof(AiActionDraft.ExecutionResultReference))] = "可追溯执行结果的非敏感引用，不保存完整工具响应",
            [(typeof(AiActionDraft), nameof(AiActionDraft.IdempotencyKey))] = "草稿生成请求的用户级幂等键",
            [(typeof(AiActionDraft), nameof(AiActionDraft.ConcurrencyVersion))] = "草稿状态流转的乐观并发版本，每次更新必须递增",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.ConversationId))] = "来源 AI 会话主键，外部 MCP 独立生成时为空",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.UserId))] = "草稿所属用户主键，也是唯一允许人工确认的用户",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.CustomerId))] = "草稿下单客户主键",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.CustomerNameSnapshot))] = "草稿生成时的客户名称快照",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.CustomerCodeSnapshot))] = "草稿生成时的客户编码快照",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.QuotationId))] = "草稿采用的默认报价单主键",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.WareId))] = "草稿指定的履约仓库主键",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.OrderDate))] = "草稿订单业务日期（UTC），用于解析有效价格",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.ReceiveDate))] = "客户要求收货时间（UTC）",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.ContactNameSnapshot))] = "草稿生成时的联系人姓名快照",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.ContactPhoneSnapshot))] = "草稿生成时的联系人电话快照",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.DeliveryAddressSnapshot))] = "草稿生成时的配送地址快照",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.Remark))] = "用户提供的订单业务备注，不保存无关内部信息",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.DraftStatus))] = "草稿状态：1 待确认，2 已确认，3 已过期，4 已取消",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.ExpiresAt))] = "草稿失效时间（UTC），默认自生成起 30 分钟",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.ConfirmedTime))] = "草稿成功创建正式订单的确认时间（UTC）",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.SaleOrderId))] = "草稿确认后创建的正式销售订单主键",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.IdempotencyKey))] = "草稿生成请求的用户级幂等键",
            [(typeof(AiOrderDraft), nameof(AiOrderDraft.ConcurrencyVersion))] = "草稿状态更新的乐观并发版本，流转时必须递增",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.AiOrderDraftId))] = "所属 AI 订单草稿主键",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.SortOrder))] = "商品行在草稿中的稳定展示顺序，从 1 开始",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.GoodsId))] = "草稿下单商品主键",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.GoodsNameSnapshot))] = "草稿生成时的商品名称快照",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.GoodsCodeSnapshot))] = "草稿生成时的商品编码快照",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.GoodsUnitId))] = "下单数量使用的商品单位主键",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.GoodsUnitNameSnapshot))] = "草稿生成时的下单单位名称快照",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.Quantity))] = "按下单商品单位计量的业务数量",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.BaseQuantity))] = "草稿生成时换算到商品基础单位的数量",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.BaseUnitId))] = "商品基础单位主键",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.BaseUnitNameSnapshot))] = "草稿生成时的基础单位名称快照",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.UnitConversion))] = "下单单位换算为基础单位的比例快照",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.FixedPrice))] = "草稿生成时解析或由用户提供的固定单价",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.FixedGoodsUnitId))] = "固定单价对应的计价单位主键",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.FixedGoodsUnitNameSnapshot))] = "草稿生成时的计价单位名称快照",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.PriceSource))] = "价格来源：1 未解析，2 客户协议价，3 默认报价，4 用户提供",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.PriceSourceRecordId))] = "协议价商品或报价商品来源记录主键",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.PriceSourceUpdatedTimeSnapshot))] = "价格来源记录在草稿生成时的最后更新时间（UTC）",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.MinimumOrderQuantitySnapshot))] = "价格来源在草稿生成时要求的最小起订数量",
            [(typeof(AiOrderDraftDetail), nameof(AiOrderDraftDetail.Remark))] = "用户提供的当前草稿商品行业务备注",
            [(typeof(McpAccessToken), nameof(McpAccessToken.UserId))] = "个人 MCP 令牌所属系统用户主键",
            [(typeof(McpAccessToken), nameof(McpAccessToken.Name))] = "用户为个人 MCP 令牌设置的用途名称",
            [(typeof(McpAccessToken), nameof(McpAccessToken.Prefix))] = "用于列表识别和令牌定位的非敏感随机前缀",
            [(typeof(McpAccessToken), nameof(McpAccessToken.TokenHash))] = "使用服务端独立密钥计算的 HMAC-SHA256 十六进制哈希",
            [(typeof(McpAccessToken), nameof(McpAccessToken.Scopes))] = "以空格分隔并按稳定顺序保存的授权范围集合",
            [(typeof(McpAccessToken), nameof(McpAccessToken.ExpiresAt))] = "个人 MCP 令牌失效时间（UTC）",
            [(typeof(McpAccessToken), nameof(McpAccessToken.RevokedTime))] = "个人 MCP 令牌被用户撤销的时间（UTC）",
            [(typeof(McpAccessToken), nameof(McpAccessToken.LastUsedTime))] = "个人 MCP 令牌最近一次通过鉴权的时间（UTC）",
