...
**P1 系统权限与基础资料、P2 订单主链路、P3 售后与财务已经完成，当前进入 P4-01 溯源模型。** 第三阶段已完成售后流程、客户账单/结款和供应商待结单据/结算单；采购入库与采购退货出库审核会自动同步供应商应付，结算单支持部分/全部结款、优惠、作废和余额回写，并由真实 HTTP 完整链路联调贯通“订单签收 -> 客户账单 -> 售后冲减 -> 客户结款”和“采购入库 -> 供应商待结单据 -> 供应商结算”两条主链路。
...
| P4 查询与支撑 | 进行中 | 当前执行 P4-01 溯源模型（检测报告、溯源记录、外部报送） |
...
按 [自动开发任务清单](./docs/自动开发任务清单.md) 顺序执行，不跳项。当前任务是 **P4-01 溯源模型**；完成并验收后再领取 P4-02。
