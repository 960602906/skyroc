...
实现状态见 [开发进度](./docs/开发进度.md)，命令结果与测试环境见 [测试进度](./docs/测试进度.md)，验收顺序见 [自动开发任务清单](./docs/自动开发任务清单.md)，业务口径见 [业务流程文档](./docs/business-flows/00-global.md)。
...
控制器位于 [SkyRoc/Controllers](./SkyRoc/Controllers/)，Swagger 在 Development 环境生成。接口统一返回 `ApiResponse<T>`，JSON 使用 camelCase；时间字段使用 System.Text.Json 默认 ISO 8601（UTC 值为 `yyyy-MM-dd'T'HH:mm:ssZ`，前端请求须带 `Z`）。

HTTP 脚本当前共记录 379 个请求，见 [SkyRoc.http](./SkyRoc/SkyRoc.http)。脚本覆盖认证、基础资料、订单、采购、库存、配送、售后、财务、溯源、报表、驾驶舱、导入导出、文件、打印和系统支撑接口。
