...
**P1 系统权限与基础资料已经完成，当前进入 P2-08 采购计划合并拆分。** 销售订单模型、事务化 CRUD、审核状态机、细粒度权限、Swagger、HTTP 主流程和 API 集成测试均已收口；采购计划模型及查询、手工新增、从已审核订单生成计划的 API 也已完成。

| 阶段 | 状态 | 范围 |
| --- | --- | --- |
| P1 系统权限与基础资料 | 已完成 | 认证、系统权限、基础资料、定价、回归测试与 API 契约 |
| P2 订单主链路 | 进行中 | 订单 CRUD 与审核联调、采购计划 API 已完成，当前执行 P2-08 计划合并拆分 |
...
采购计划的合并/拆分、采购单、库存单据、配送、售后、财务、报表和溯源 API 尚未实现。

HTTP 脚本当前共记录 186 个请求；第一阶段真实路由、响应约定和权限编码见 [第一阶段 API 契约](./docs/第一阶段API契约.md)。
...
按 [自动开发任务清单](./docs/自动开发任务清单.md) 顺序执行，不跳项。当前任务是 **P2-08 采购计划合并拆分**；完成并验收后再领取 P2-09。
