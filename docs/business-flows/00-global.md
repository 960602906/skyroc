...
文件上传采用 RustFS（S3 兼容）对象存储与元数据表，不能通过静态路径或对象键直接访问。上传时同时验证扩展名、客户端 MIME 类型和文件签名，仅支持 PDF、PNG、JPEG，单文件不超过 10 MiB；服务端生成随机存储键写入对象存储，不使用用户给出的路径或文件名作为实际键。下载由 API 代理并仅限创建人，并把其他用户、丢失对象和不存在主键统一表现为未找到。
...
- 后端 OpenAPI 已加载 Web、Application、Domain 和 Shared 的 XML 注释；控制器使用强类型 `ActionResult<ApiResponse<T>>`，Swagger 回归测试持续校验请求体、响应体、权限和业务 Tag。
- 当前导入导出只发布 `1 = Goods`。以后新增 `jobType` 时必须同时补齐格式校验、权限、Swagger、HTTP 示例和本业务文档，不能把未实现类型提前写入公开契约。
- 订单、采购、库存、配送、售后和结算的合法状态流转均由应用服务校验，非法状态和重复业务动作必须返回业务异常；各模块文档记录对应状态与下游副作用。
- 数量、价格、成本和金额统一使用 `Shared.Constants.NumericPrecision`，服务和 DTO 不得自行硬编码小数位或舍入模式。
- 库存审核与反审核通过只追加正向/反向流水保留历史；结算作废回滚来源余额并保留凭证；删除仅允许处于可删除状态的草稿或已作废记录，不能物理抹除已生效业务事实。
