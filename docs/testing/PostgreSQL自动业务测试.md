...
全量验收入口（默认**不**跑完整约 50 分钟 `SkyRoc.Tests`）：
...
1. 校验 `Testing` 环境与精确库名 `skyroc`；
2. `dotnet ef database update`；
3. `dotnet ef migrations has-pending-model-changes`（有挂起模型变更则失败）；
4. 构建到临时输出目录后运行精确 filter：`FullAcceptancePostgreSqlTests` 与质量/元数据/白名单守卫；
5. `dotnet format SkyRoc.sln --verify-no-changes`。

仅当用户明确要求手动触发完整回归时，才追加：

```powershell
.\scripts\Invoke-SkyRocFullAcceptance.ps1 -IncludeFullTestSuite
```

`FullAcceptancePostgreSqlTests` 会生成最终 JSON/Markdown 质量报告，断言 `demoDataAcceptance.isReady=true`、无临时残留，并核对 `SKYROC-DEMO-*` 仓库/商品/客户/供应商/系统用户等联调稳定键可查询。前端账号与稳定键约定见 [前端联调数据说明.md](./前端联调数据说明.md)。
