...
/// <summary>
/// 其他入库控制器，提供手工增加库存的查询、维护及审核、反审核接口。
/// </summary>
[ApiController]
[Route("api/stock-in/other")]
[Authorize]
[PermissionResource(PermissionCodes.Business.Storage.Resource)]
public class OtherStockInController(IStockInService service) : ControllerBase
{
    private const StockInOrderType OrderType = StockInOrderType.Other;

    /// <summary>
    /// 分页查询其他入库单及商品明细。需要库存读取权限。
    /// </summary>
    /// <param name="parameters">其他入库分页和筛选参数。</param>
    /// <returns>其他入库单分页结果。</returns>
    [HttpGet("list")]
    ...
    /// 查询其他入库单详情。需要库存读取权限。
    /// </summary>
    /// <param name="id">其他入库单主键。</param>
    /// <returns>其他入库单完整详情。</returns>
    [HttpGet("{id:guid}")]
    [ResourcePermission(PermissionActions.Read)]
    ...
    public async Task<ActionResult<ApiResponse<StockInOrderDto>>> Create([FromBody] CreateOtherStockInDto dto)
    {
    ...
    public async Task<ActionResult<ApiResponse<StockInOrderDto>>> Update([FromBody] UpdateOtherStockInDto dto)
    {
