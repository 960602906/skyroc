...
public class OperationAuditMiddleware(RequestDelegate next, ILogger<OperationAuditMiddleware> logger)
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        finally
        {
            ...
            try
            {
                ...
                {
                    Module = GetModule(request.Path), OperationType = GetOperationType(request.Method),
                    Description = $"{request.Method} {request.Path}", Method = request.Method, Url = request.Path,
                    RequestSummary = RedactQuery(request.Query), ResponseSummary = $"HTTP {statusCode}",
                    IpAddress = context.Connection.RemoteIpAddress?.ToString() ?? string.Empty,
                    UserAgent = request.Headers.UserAgent.ToString(), ExecutionDuration = watch.ElapsedMilliseconds,
    ...
    private static bool ShouldAudit(HttpRequest request) =>
        request.Path.StartsWithSegments("/api") && request.Method is "POST" or "PUT" or "PATCH" or "DELETE";
    ...
    private static bool IsSensitive(string key) => key.Contains("password", StringComparison.OrdinalIgnoreCase) || key.Contains("token", StringComparison.OrdinalIgnoreCase) || key.Contains("secret", StringComparison.OrdinalIgnoreCase);
}
